...
const handleProviderApproval = async (id: string, status: 'APPROVED' | 'REJECTED') => {
 ...
 const BusinessReviewsContent = () => (
  <div className="space-y-6 p-6">
    <div className="flex items-center justify-between">
      <h2 className="text-3xl font-bold text-gray-900">Provider Applications</h2>
      <div className="text-sm text-gray-500">
        {providers.length} application{providers.length !== 1 ? 's' : ''}
      </div>
    </div>
          ...
          <tbody className="divide-y divide-gray-100">
            {providers.length > 0 ? providers.map(provider => (
              <tr key={provider.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="py-4 px-6">
                  <div className="space-y-1">
                    <p className="text-gray-800 font-medium">{provider.email}</p>
                  ...
                  {provider.status === 'PENDING' && (
                    <div className="flex gap-2 justify-end">
                      <button
                        ...
                        className="inline-flex items-center gap-2 bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors shadow-sm"
                      >
                        👁️ View
                      </button>
                      <ActionButton
              ...
              <tr>
                <td colSpan={5} className="text-center py-12">
                  <div className="flex flex-col items-center space-y-3">
                    ...
                    <p className="text-gray-500 font-medium">No provider applications found.</p>
