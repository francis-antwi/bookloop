...
if (confidence >= 80) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-md p-2 sm:p-4">
      <div className="relative w-full max-w-lg bg-white rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden transform transition-all duration-300 max-h-[95vh] flex flex-col">
        
        {/* Header */}
        <div className="relative bg-gradient-to-r from-blue-600 to-purple-600 px-4 sm:px-8 py-4 sm:py-6 text-white flex-shrink-0">
          <button 
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="px-4 sm:px-8 py-4 sm:py-6">
            ...
            <div className="text-center mb-6 sm:mb-8">
              <div className="inline-flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-gradient-to-r from-blue-100 to-purple-100 rounded-xl sm:rounded-2xl mb-3 sm:mb-4">
              ...
              {currentField.field === 'role' ? (
                ...
                <div className="space-y-4">
                  <div className="border-2 border-dashed border-gray-300 rounded-xl p-6 sm:p-8 text-center hover:border-blue-400 transition-colors duration-200">
                    <FiUpload className="w-8 h-8 sm:w-12 sm:h-12 text-gray-400 mx-auto mb-3 sm:mb-4" />
                    <input
                      type="file"
                      accept="image/*"
                  ...
                  {idFile && (
                    <div className="flex items-start gap-2 text-green-600 bg-green-50 p-3 rounded-xl">
                      <FiCheck className="w-5 h-5 flex-shrink-0 mt-0.5" />
                      <div className="min-w-0 flex-1">
                        <span className="text-sm font-medium block">ID selected:</span>
                        <span className="text-sm text-green-700 break-all">{idFile.name}</span>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2">
                  <div className="relative">
                    <div className={`flex items-center border-2 rounded-xl px-3 sm:px-4 py-3 transition-all duration-200 ${
                      ...
                      <input
                        ...
                        className="flex-1 bg-transparent outline-none text-gray-800 placeholder-gray-400 min-w-0 text-sm sm:text-base"
                        disabled={isLoading}
                      />
                      {currentField.field === 'password' && (
                        <button
                          ...
                          className="p-1 text-gray-400 hover:text-gray-600 transition-colors duration-200 flex-shrink-0"
                        >
                  ...
                  {errors[currentField.field] && (
                    <p className="text-red-500 text-sm flex items-start gap-2 mt-2">
                      <span className="w-1 h-1 bg-red-500 rounded-full mt-2 flex-shrink-0"></span>
                      <span className="break-words">{errors[currentField.field]?.message}</span>
                    </p>
                  )}
                </div>
              )}
              ...
              <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-red-50 border border-red-200 rounded-xl">
                <div className="flex items-start gap-2 text-red-700">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="text-sm break-words">Match score: {matchStatus.faceConfidence.toFixed(1)}% (minimum required: 80%)</p>
                  </div>
                </div>
              </div>
            )}
        ...
        <div className="px-4 sm:px-8 py-4 sm:py-6 bg-gray-50 border-t border-gray-100 flex-shrink-0">
          <div className="flex justify-between items-center gap-3">
            {currentStep > 0 ? (
              <button
                onClick={handlePrev}
                disabled={isLoading}
                className="flex items-center gap-2 px-4 sm:px-6 py-2 text-gray-600 hover:text-gray-800 transition-colors duration-200 text-sm sm:text-base"
              >
            ...
            {currentStep < steps.length - 1 ? (
              <button
                ...
                className={`flex items-center gap-2 px-4 sm:px-8 py-2 sm:py-3 rounded-xl font-medium transition-all duration-200 text-sm sm:text-base ${
