...
  // State to hold all collected data before final registration
...
      verificationFormData.append('verificationStep', 'identity'); // Indicate this is identity step
...
      businessUploadFormData.append('verificationStep', 'business'); // Indicate this is business step
      
      // Append business files for upload
      Object.entries(businessFiles).forEach(([key, file]) => {
        if (file) businessUploadFormData.append(key, file);
      });

      // Send business files to /api/verify for upload only
...
      router.push('/pending-approval'); // Or whatever your next route is
      onComplete(); // Call the onComplete prop if needed
    } catch (error: any) {
...
      setCurrentStep('id'); // Go back to ID step if from business
    }
  };
...
                </div>
                <span className="mt-1 text-xs text-gray-600">Selfie</span>
              </div>
            </div>
            
...
                </div>
                <span className="mt-1 text-xs text-gray-600">ID Document</span>
              </div>
            </div>
            
            <div className="flex items-center">
...
                  3
                </div>
                <span className="mt-1 text-xs text-gray-600">Business</span>
              </div>
            </div>
          </div>
          
...
                  Match confidence: {identityVerificationStatus.confidence}% - Moving to business verification...
                </p>
              </div>
            </div>
          </div>
